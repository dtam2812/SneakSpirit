  ...
  const handleAddVoucher = async () => {
    try {
      ...
      if (
        ...
        setListVoucher((prev) => [...prev, response.data]);
        setIsAdding(false);
      }
    } catch (error) {
      console.log(error);
  ...
  const handleUpdateVoucher = async (voucherId) => {
    try {
      const response = await axios.put(
        `auth/admin/voucher/update/${voucherId}`,
        {
      ...
      setListVoucher(
        (prev) => prev.map((v) => (v._id === voucherId ? response.data : v)), // ← cập nhật item trong list
      );
              ...
              <div>
                ...
                <div className="flex justify-around mt-8 gap-x-2">
                  ...
                  {isEditing === false ? (
                    ...
                    <button
                      onClick={() => handleUpdateVoucher(value.idValue)}
                      className="border border-gray-800 w-1/2 bg-black text-white hover:bg-gray-700 transition-colors py-2 px-6 rounded-lg"
                    >
                      Cập nhật
                    </button>
