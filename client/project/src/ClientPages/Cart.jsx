...
const Cart = () => {
      ...
      if (voucher.type === "Giảm giá") {
        if (voucher.discountType === "Phần trăm") {
          priceDiscount = Math.min(
            (billPrice * voucher.discount) / 100,
        ...
        if (voucher.discountType === "Phần trăm") {
          shippingFeeDiscount = Math.min(
            (fee * voucher.discount) / 100,
            voucher.maximumDiscount,
          );
        } else {
          shippingFeeDiscount = Math.min(
  ...
  const applyingVoucher = (e) => {
    ...
    const foundVoucher = listVoucher.find(
      (element) => element.voucherName === discountInput,
    );
    ...
    if (isProductVoucher) {
      newVouchers = newVouchers.filter(
        (element) => element.type !== "Giảm giá",
      );
    } else if (isShippingVoucher) {
      newVouchers = newVouchers.filter(
        (element) => element.type !== "Giao hàng",
      );
    }
        ...
        <section className="bg-white py-8 antialiased dark:bg-gray-900 md:py-8">
          <div className="mx-auto max-w-screen-xl px-4 2xl:px-0">
            ...
            </h2>
            {
              //Liệt kê sản phẩm
              cartList.length > 0 ? (
                <div className="mt-6 text-start sm:mt-8 md:gap-6 lg:flex lg:items-start xl:gap-8">
                              ...
                              <div className=" sm:flex sm:items-center sm:justify-between">
                                <div className="flex items-center gap-x-5 mr-3 mb-3 sm:mb-0">
                                  ...
                                  <div>
                                    <Link to={`/product/${element.id}`}>
                                      <p className="font-semibold text-gray-900 hover:underline dark:text-white cursor-pointer">
                                        {element.name}
                                      </p>
                                    </Link>
                                  </div>
                                ...
                                <div className="flex items-center justify-between gap-x-6">
                                  ...
                                  <button
                                    ...
                                    type="button"
                                    className="py-3 px-4 border border-red-600 hover:bg-red-600 hover:text-white duration-300 rounded-2xl text-sm font-medium text-red-600"
                                  >
                                    Xóa
                                  </button>
                                </div>
                    ...
                    <div className="space-y-4 rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800 sm:p-6">
                        ...
                        <div className="space-y-2">
                          ...
                          {bill.shippingFeeDiscount !== 0 && (
                            <dl className="flex items-center justify-between gap-4">
                              ...
                              <dd className="text-base font-medium text-gray-900 dark:text-white">
                                {" "}
                                {bill.shippingFeeDiscount.toLocaleString(
                                  "vi-VN",
                                )}
                                đ
                              </dd>
                      ...
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <Link to={"/collection"}>
                          <p
                            title
                            className="cursor-pointer inline-flex items-center gap-2 text-sm font-medium text-primary-700 underline hover:no-underline "
                          >
                            Tiếp tục mua sắm
                            <svg
                              className="h-5 w-5"
                ...
                <div className="space-y-4 text-center mx-auto py-5">
                  <img
                    className="mx-auto"
                    src="https://bizweb.dktcdn.net/100/472/913/themes/888429/assets/cart_empty_background.png?1725935235961"
                  />
                  <h2 className="text-3xl">"Hổng" có gì trong giỏ hàng hết</h2>
                  <p>Về trang cửa hàng để chọn mua sản phẩm bạn nhé !!!</p>
                  <Link to={"/collection"}>
                    <button className="py-3 my-4 px-5 text-xl font-semibold border-2 border-[#919191] rounded-lg text-[#919191] hover:bg-[#919191] hover:text-white  duration-300">
                      Mua sắm ngay
                    </button>
                  </Link>
                </div>
